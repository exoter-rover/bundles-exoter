...
options[:visual_stereo] = false
options[:gp] = false

op = OptionParser.new do |opt|
    opt.banner = <<-EOD
    usage: process_logs_exoter_vsd_slam.rb [options] <data_log_directory>
    EOD
    ...
    opt.on "-v", "--visual_stereo=task/log", String, 'select visual stereo from a running task or from the log files' do |visual|
        options[:visual_stereo] = visual
    end
...
if options[:visual_stereo].casecmp("task").zero?
    puts "[INFO] Visual Stereo task running"
elsif options[:visual_stereo].casecmp("log").zero?
    puts "[INFO] Visual Stereo from log files"
else
    puts "[INFO] No Visual Stereo selected! EXIT"
    exit 1
end
...
Bundles.run 'exoter_control',
    ...
    vsd_slam = Orocos.name_service.get 'vsd_slam'
    ...
    Orocos.conf.apply(vsd_slam, ['default', 'bumblebee'], :override => true)
    ...
    Bundles.transformer.setup(vsd_slam)
    ...
    # Visual stereo connections
    if options[:visual_stereo].casecmp("task").zero?
        log_replay.camera_bb2.left_frame.connect_to visual_stereo.left_frame, :type => :buffer, :size => 1000
        log_replay.camera_bb2.right_frame.connect_to visual_stereo.right_frame, :type => :buffer, :size => 1000
    end
    ...
    if options[:odometry].casecmp("task").zero?
        ...
        if options[:gp]
            ...
            gp_odometry.delta_pose_samples_out.connect_to vsd_slam.delta_pose_samples,  :type => :buffer, :size => 1000
        else
            exoter_odometry.delta_pose_samples_out.connect_to vsd_slam.delta_pose_samples,  :type => :buffer, :size => 1000
        end
        ...
        if options[:gp]
            ...
            # SLAM odometry poses
            log_replay.exoter_odometry.delta_pose_samples_out.connect_to(vsd_slam.delta_pose_samples, :type => :buffer, :size => 1000)
        end
    end

    if options[:visual_stereo].casecmp("task").zero?
        # Visual stereo measurement to SLAM
        visual_stereo.features_samples_out.connect_to vsd_slam.visual_features_samples, :type => :buffer, :size => 100
    elsif options[:visual_stereo].casecmp("log").zero?
        # Visual stereo measurement to SLAM
